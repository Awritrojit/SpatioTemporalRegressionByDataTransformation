...
1.  **Configuration (`cfg.yml`)**: Define all parameters for data generation, transformation, and regression.
2.  **Data Generation (`SpatioTemporalData.py`)**:
    ```bash
    python SpatioTemporalData.py
    ```
    This generates `low_res_data/`, `high_res_data/`, and `bias.npy` in the `data/` directory.
3.  **Data Transformation (`data_transform.py`)**:
    ```bash
    python data_transform.py
    ```
    This processes the raw data and creates the `train_X/`, `train_y/`, `test_X/`, and `test_y/` datasets in the `data/` directory.
4.  **Model Training and Evaluation (`str.py`)**:
    ```bash
    python str.py
    ```
    This script loads the transformed data, trains the specified regressor, evaluates it on the test set, prints metrics, and saves the training history and a prediction visualization.
